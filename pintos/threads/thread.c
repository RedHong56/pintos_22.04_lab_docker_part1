...
void thread_tick(int64_t ticks)
{
	...
	if (ticks % 4 == 0) 
	{
		...
		if (!list_empty(&ready_list)) 
		{
			...
			for (; e != list_end(&ready_list); e = e->next)
			{
				struct thread *temp_t = list_entry(e, struct thread, elem);
				if (max_t->priority< temp_t->priority)
					max_t = temp_t;
			}
	...
	/* Enforce preemption. */ //CPU 독점 방지 타이머 time slice = 4
...
void mlfqs_update_load_avg(void){ 
	int t_num = int_to_fp(list_size(&ready_list) + (thread_current() == idle_thread ? 0 : 1) );
	load_avg = fp_add_fp(fp_mul_int(fp_div_int(load_avg,60), 59) , fp_div_int(t_num, 60));
}

void mlfqs_update_all_recent_cpu(void){
	int decay = fp_div_fp(fp_mul_int(load_avg, 2), fp_add_int(fp_mul_int(load_avg,2), 1)); //(2*load_avg)/(2*load_avg+1)

	// all list를 돌며 thread.recent_cpu = (decay*recent_cpu) + nice
	for (struct list_elem *e = list_begin(&all_list); e != list_end(&all_list); e = e->next)
	{
		struct thread *t = list_entry(e, struct thread, all_elem);
		t->recent_cpu = fp_add_int(fp_mul_fp(decay, t->recent_cpu),t->nice);
	}
}

void mlfqs_update_all_priority (void){
	//all list 순회 thread.priority= PRI_MAX - (recent_cpu/4) - (nice *2)
		for (struct list_elem *e = list_begin(&all_list); e != list_end(&all_list); e = e->next)
	{
		struct thread *t = list_entry(e, struct thread, all_elem);
		t->priority = PRI_MAX - fp_to_int(fp_div_int(t->recent_cpu, 4)) - t->nice*2;
	}
}
//////////////////////////////////////////////////////load_avg 갱신////////////////////////////////////////////////////////////////////
...
/*
 * [7단계] 현재 스레드의 nice 값을 반환합니다.
 * 이 함수는 MLFQS 모드에서만 의미가 있습니다.
 */
int
thread_get_nice (void)
{
    /* 스레드 구조체에서 nice 값을 읽어 반환합니다. */
    /* 이 작업은 원자적이므로 별도 락이 필요 없습니다. */
    return thread_current ()->nice;
}

/*
 * [7단계] 현재 스레드의 nice 값을 설정합니다.
 *
 * nice 값은 [NICE_MIN, NICE_MAX] 범위로 제한됩니다.
 * 값 변경 즉시, 이 스레드의 priority를 재계산해야 하며,
 * 만약 이 스레드가 더 이상 1등이 아니라면 CPU를 양보(yield)해야 합니다.
 */
void
thread_set_nice (int new_nice)
{
    /* MLFQS 모드가 아니라면 아무것도 하지 않습니다. */
    if (!thread_mlfqs) {
        return;
    }

    /* * nice 값 변경, priority 재계산, yield 검사는
     * 모두 원자적(atomic)으로 일어나야 합니다.
     * (인터럽트를 비활성화합니다)
     */
    enum intr_level old_level = intr_disable ();
    ...
    /* 1. nice 값을 [-20, 20] 범위로 제한(clamp)하여 설정합니다. */
    ...
    /* * 2. nice 값이 바뀌었으므로, 이 스레드의 priority를 즉시 재계산합니다.
     * (4틱 로직인 mlfqs_update_all_priority 함수 내부의 공식과 동일합니다)
     */
    ...
    /* priority도 [0, 63] 범위로 제한(clamp)합니다. */
    ...
    /*
     * 3. (선점 검사) 만약 나의 우선순위가 낮아졌다면,
     * ready_list의 1등(max)과 비교하여 양보(yield)할지 결정합니다.
     *
     * (이 코드는 thread_tick의 4틱 검사 로직과 동일합니다)
     */
    if (old_priority > new_priority && !list_empty (&ready_list))
    {
        /* ready_list의 "진짜 1등"을 찾습니다 (정렬되어 있지 않으므로) */
        struct list_elem *e = list_begin (&ready_list);
        ...
        /* * 현재 스레드가 "진짜 1등"보다 우선순위가 낮아졌다면 양보합니다.
         * (이곳은 인터럽트 문맥이 아니므로 thread_yield()를 사용합니다)
         */
...
/*
 * [7단계] 시스템 전역 load_avg 값을 반환합니다.
 *
 * (명세 요구사항)
 * 고정소수점(fp) 값을 그대로 반환하는 것이 아니라,
 * 100을 곱하고 "반올림"하여 "일반 정수(int)"로 반환합니다.
 */
int
thread_get_load_avg (void)
{
    /* 이 함수는 thread.c에 있어야만 static 변수인 load_avg에 접근 가능 */
    /* load_avg (fp) * 100 (int) */
    int load_avg_x_100 = fp_mul_int (load_avg, 100);
    
    /* 결과를 반올림하여 int로 반환 */
    return fp_to_int_round (load_avg_x_100);
}

/*
 * [7단계] 현재 스레드의 recent_cpu 값을 반환합니다.
 *
 * (명세 요구사항)
 * 고정소수점(fp) 값을 그대로 반환하는 것이 아니라,
 * 100을 곱하고 "반올림"하여 "일반 정수(int)"로 반환합니다.
...
{
    /* recent_cpu (fp) * 100 (int) */
    int recent_cpu_x_100 = fp_mul_int (thread_current ()->recent_cpu, 100);

    /* 결과를 반올림하여 int로 반환 */
    return fp_to_int_round (recent_cpu_x_100);
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
init_thread(struct thread *t, const char *name, int priority)
{
	...
	// all list 추가
	list_insert(&all_list, &t->all_elem);
}
